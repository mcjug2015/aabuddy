...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        query = '''
            CREATE VIEW aabuddy_activenottheresview AS
            select mnt.unique_phone_id as id, max(mnt.user_id) as user_id, mnt.unique_phone_id as unique_phone_id, count(mnt.meeting_id) as total, count(distinct mnt.meeting_id) as distinct, max(mnt.created_date) as created_date
            from aabuddy_meetingnotthere mnt
            group by mnt.unique_phone_id
            order by max(mnt.created_date) desc;
        '''
        db.execute(query)
        db.send_create_signal(u'aabuddy', ['ActiveNotTheresView'])

    def backwards(self, orm):
        db.execute("DROP VIEW aabuddy_activenottheresview;")
    ...
    models = {
        ...
        'aabuddy.notthereview': {
            'Meta': {'object_name': 'NotThereView'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'latest_not_there': ('django.db.models.fields.DateTimeField', [], {}),
            'meeting': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['aabuddy.Meeting']"}),
        ...
        'aabuddy.userconfirmation': {
            ...
            'created_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 5, 17, 0, 0)'}),
