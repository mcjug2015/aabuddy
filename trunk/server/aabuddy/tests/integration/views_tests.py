...
class TestViews(TestCase):
    ...
    def test_get_meeting_by_id_happy_path(self):
        resp = self.client.get('/meetingfinder/get_meeting_by_id', {'meeting_id': 1})
    ...
    def test_send_reset_conf_get(self):
        resp = self.client.get('/meetingfinder/send_reset_conf', {'username': 'papa_nurgle'})
        self.assertEqual(resp.status_code, 400)
    ...
    def test_create_user_post(self):
        ...
        when(views).send_email_to_user(any(), any(), any()).thenReturn(None)
        resp = self.client.post('/meetingfinder/create_user', {'username': 'papa_nurgle', 'password': 'I am a cow'})
        self.assertEqual(resp.status_code, 200)
        verify(views, times=1).send_email_to_user(any(), any(), any())
    ...
    def test_create_user_post_no_username(self):
        resp = self.client.post('/meetingfinder/create_user', {})
        self.assertEqual(resp.status_code, 401)
    ...
    def test_create_user_post_taken_username(self):
        ...
        when(views).send_email_to_user(any(), any(), any()).thenReturn(None)
        resp = self.client.post('/meetingfinder/create_user', {'username': 'test_user', 'password': 'moooo'})
        self.assertEqual(resp.status_code, 401)
        self.assertEqual(resp.content, "You must pass in an untaken username and a non-empty password.")
    ...
    def test_create_user_get(self):
        ...
        uc.save()
        resp = self.client.get('/meetingfinder/create_user', {'confirmation': 'mooo123'})
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(User.objects.get(username='test_user').is_active)
    
    def test_create_user_get_no_conf(self):
        resp = self.client.get('/meetingfinder/create_user')
        self.assertEqual(resp.status_code, 400)
    ...
    def test_change_password(self):
        ...
        resp = self.client.post('/meetingfinder/validate_user_creds')
        self.assertEqual(resp.status_code, 401)
        
        credentials = base64.b64encode('test_user:1chpok1')
        self.client.defaults['HTTP_AUTHORIZATION'] = 'Basic ' + credentials
        resp = self.client.post('/meetingfinder/change_password', {'new_password': 'oooAAA'})
        self.assertEqual(resp.status_code, 200)
    ...
    def test_change_password_blank(self):
        credentials = base64.b64encode('test_user:1chpok1')
        self.client.defaults['HTTP_AUTHORIZATION'] = 'Basic ' + credentials
        resp = self.client.post('/meetingfinder/change_password')
        self.assertEqual(resp.status_code, 401)
    ...
    def test_change_password_get(self):
        resp = self.client.get('/meetingfinder/change_password')
        self.assertEqual(resp.status_code, 400)
...
class TestResetPassword(TestCase):
    ...
    def test_reset_password_form_password_mismatch(self):
        resp = self.client.post("/meetingfinder/reset_password/", {"new_password": "aaaa",
    ...
    def test_get_happy_path(self):
        uc = UserConfirmation(user=User.objects.get(username='test_user'),
                              ...
                              confirmation_key='mooo123')
        uc.save()
        resp = self.client.get("/meetingfinder/reset_password/", {"confirmation": "mooo123"})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context['form'].fields['user_confirmation'].widget.attrs['value'], "mooo123")
        
    def test_get_no_user_conf(self):
        resp = self.client.get("/meetingfinder/reset_password/")
        self.assertEqual(resp.status_code, 401)
        self.assertIn("User confirmation is invalid, expired or does not exist", resp.content)
...
class FindSimilarTest(TestCase):
    ...
    def test_happy_path(self):
        resp = self.client.post('/meetingfinder/find_similar', json.dumps({'day_of_week': 1,
                                                          'start_time': '11:51:00',
                                                          'end_time': '13:09:00',
                                                          'name': 'mooo',
                                                          'description': 'test',
